...
# -*- coding: utf-8 -*-
"""
分析生成的水印 .ppt 文件，检查水印 SpContainer 是否被正确插入。
"""
...
def find_all_spcontainers(data, spgr_offset):
    """在 SpgrContainer 中找到所有 SpContainer。"""
    spcontainers = []
...
def dump_spcontainer(data, sp_offset, indent=0):
    ...
    print(f"{prefix}SpContainer (offset=0x{sp_offset:X}, len={rec_len}):")
    ...
    while pos + 8 <= sp_end:
        ...
        if rec_type == RT_FSP and rec_len >= 8:
            ...
            print(f"{prefix}    FSP: inst=0x{inst:X} shapeId={shape_id} flags=0x{flags:08X}")
...
def analyze_file(filepath):
    ...
    # 分析每个 Slide 的 SpContainer
    for i, slide_offset in enumerate(slides):
        ...
        # dump 最后一个 SpContainer（应该是水印）
        if spcontainers:
            last_sp = spcontainers[-1]
            print(f"\n  最后一个 SpContainer（可能是水印）:")
            dump_spcontainer(ppt_data, last_sp, 2)
...
def main():
    ...
    for fname in sorted(os.listdir(test_dir)):
        if fname.lower().startswith("wm_") and fname.lower().endswith(".ppt"):
